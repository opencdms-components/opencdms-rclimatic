...
# start_month, the month (1 to 12) to start the year in.
# Default is 1 to start on 1 January.
...
def year_num(dates, start_month=1):
    ...
    ro.globalenv["start_month"] = _start_month

    r(
        """

    library("lubridate")
    if (!start_month %in% 1:12) stop(
        "start_month must be an integer between 1 and 12. ",
        start_month,
        " is invalid."
    )
    year_col <- lubridate::year(dates)
    if (start_month > 1) {
        # Using a leap year as year to
        # ensure consistent day of year across years.
        start_doy <- lubridate::yday(
            as.Date(paste("2000", start_month, 1), format = "%Y %m %d")
        )
