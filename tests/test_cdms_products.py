...
def test_histogram_plot(caplog):
    data_file: str = os.path.join(TEST_DIR, "data", "niger50.csv")
    niger50 = read_csv(
        data_file,
        parse_dates=["date"],
    ...
    # TODO
    # should generate warning 'facet_by set to none since no stations are given in data'.
    # Note: The warning is written to the Python Test Log. Pytest has features to catch and check for warnings 
    # (see https://docs.pytest.org/en/6.2.x/warnings.html) but this does not catch the warnings generated by R.
    # For example, the code below creates a record object with zero elements.:
    # ```
    ...
    #          :
    # ```
    # I raised an issue for this: https://github.com/rpy2/rpy2/issues/894
    # Also see _WRITECONSOLE_EXCEPTION_LOG in `callbacks.py` at https://github.dev/rpy2/rpy2/tree/master/rpy2
    # Python logging info: https://docs.python.org/3/library/logging.html#
    # Another approach would be to write the R output to file in the R environment, e.g.:
    #   https://stackoverflow.com/questions/60636015/how-to-save-the-console-in-r-to-a-file
    # Note: for this test to pass, it may need special niger50 data, see C:\Users\steph\OneDrive\Desktop\FirefoxDownloads\CDMS niger50 data for histogram_plot warning test 
    # histogram_plot(data = niger50, date_time = "date", elements = "tmax", facet_by = "stations")
    # file_name_actual: str = "histogram_plot_actual100.jpg"
